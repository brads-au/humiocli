...
type EventForwarder struct {
	...
	Name          string
	Description   string
	ForwarderInfo struct {
		Properties string
		Topic      string
		Enabled    bool
	} `graphql:"... on KafkaEventForwarder"`
}

func (c *Client) EventForwarders() *EventForwarders { return &EventForwarders{client: c} }

// func (e *EventForwarders) Get(name string) (EventForwarder, error) {
// 	var query struct {
...
func (e *EventForwarders) List() ([]EventForwarder, error) {
	var query struct {
		EventForwarders []EventForwarder `graphql:"eventForwarders"`
	}

	err := e.client.Query(&query, nil)
	return query.EventForwarders, err
}
...
func (e *EventForwarders) Create(name, description, properties, topic string, enabled bool) error {
	var mutation struct {
		Result struct {
			// We have to make a selection, so just take __typename
			Typename graphql.String `graphql:"__typename"`
		} `graphql:"createKafkaEventForwarder(input: {name: $name, description: $description, properties: $properties, topic: $topic, enabled: $enabled})"`
	}

	variables := map[string]interface{}{
		"name":        graphql.String(name),
		"description": graphql.String(description),
		"properties":  graphql.String(properties),
		"topic":       graphql.String(topic),
		"enabled":     graphql.Boolean(enabled),
	}

	err := e.client.Mutate(&mutation, variables)
	if err != nil {
		// The graphql error message is vague if the repo already exists, so add a hint.
		return fmt.Errorf("%w. Does the event forwarder already exist?", err)
	}
